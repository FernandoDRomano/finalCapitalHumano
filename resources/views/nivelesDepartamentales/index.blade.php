...
<div class="card">
    <div class="card-header blue-marino d-flex justify-content-between align-items-center">
      <h3 class="card-title flex-grow-1"><strong><i class="fas fa-list"></i> <span class="mx-2 h4">Gestión de Niveles Departamentales</span></strong></h3>
      <a id="btnAgregar" type="button" class="btn btn-primary float-right" href="#" data-toggle="modal" data-target="#modalAgregar"
      data-tooltip="tooltip" data-placement="top" title="Agregar Nuevo Nivel Departamental al Sistema">
        ...
        @foreach ($nivelesDepartamentales as $nivel)
            <tr>
                ...
                <td>
                    ...
                    data-tooltip="tooltip" data-placement="top" title="Editar el Nivel Departamental del Sistema" data-id="{{$nivel->id}}"><i class="fas fa-edit" data-id="{{$nivel->id}}"></i></a>
                    <a name="btnEliminar" class="btn btn-danger text-white eliminar" href="#" role="button" data-toggle="modal" data-target="#modalEliminar"
                    data-tooltip="tooltip" data-placement="top" title="Eliminar el Nivel Departamental del Sistema" data-id="{{$nivel->id}}"><i class="fas fa-trash-alt" data-id="{{$nivel->id}}"></i></a>
                </td>
            </tr>
        @endforeach
        </tbody>
...
<div class="modal" tabindex="-1" role="dialog" id="modalEliminar">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            {{method_field('delete')}}
                <div class="modal-body">
                    <div id="contenidoModalEliminar" class="card-body"></div>
...
<script>
   ...
   function pintarModal(id, nivel){
            ...
            const url = `getDepartamentosDependientes/${id}`;
            //LLAMADA A FETCH
            fetch(url, {
                headers: {
                "Content-Type": "application/json",
            ...
            .then(datos => {
                ...
                if(datos.departamentos.length > 0){
                    ...
                    let template = `<h4 class="text-center text-danger">Error: No se podra eliminar este Nivel Departamental,
                    debido a que esta siendo utilizado por
                    <Strong>
                    ${datos.departamentos.length == 1 ? datos.departamentos.length + ' departamento' : datos.departamentos.length + ' departamentos'}</strong></h4>`;
                    ...
                    const contenido = document.getElementById('contenidoModalEliminar');
                    contenido.innerHTML = `<h4 class="text-center">¿Estas seguro de Eliminar al Nivel Departamental <strong>${nivel.nombre}</strong>?</h4>`;

                    formEliminar.setAttribute('action', 'nivelesDepartamentales/' + nivel.id);
